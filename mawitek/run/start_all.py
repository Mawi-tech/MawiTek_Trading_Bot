...
COMPONENTS: dict[str, dict] = {
    "dashboard": {
        "module":  "mawitek.dashboard.dashboard_server",
        "args":    ["--no-browser"],
        "label":   "Dashboard server",
        "needs_broker": False,
    },
    "executor": {
        "module":  "mawitek.strategies.executor",
        "args":    [],
        "label":   "Strategy 1 — Swing options executor",
        "needs_broker": True,
    },
    "iv_rank_bot": {
        "module":  "mawitek.strategies.iv_rank_bot",
        "args":    [],
        "label":   "Strategy 2 — IV rank bot",
        "needs_broker": True,
    },
    "hft_executor": {
        "module":  "mawitek.strategies.hft_executor",
        "args":    [],
        "label":   "Strategy 3 — Intraday HFT executor",
        "needs_broker": True,
    },
    "pead_executor": {
        "module":  "mawitek.strategies.pead_executor",
        "args":    [],
        "label":   "Strategy 4 — Post-earnings / news-drift executor",
        "needs_broker": True,
    },
    "bounce_executor": {
        "module":  "mawitek.strategies.bounce_executor",
        "args":    [],
        "label":   "Strategy 5 — Capitulation-bounce executor (bear-regime offense)",
        "needs_broker": True,
    },
    "news_monitor": {
        "module":  "mawitek.data.news_feed",
        "args":    ["--monitor"],
...
ROOT = Path(__file__).resolve().parents[2]   # project root (mawitek/run/ -> up 2)
...
def _spawn(name: str, comp: dict, logs_dir: Path) -> subprocess.Popen:
    ...
    cmd = [sys.executable, "-m", module, *comp["args"]]
...
def main() -> int:
    ...
    try:
        from mawitek.infra.heartbeat import clear as _hb_clear
    ...
    try:
        ...
        while True:
            ...
            if (watched
                ...
                last_hb_check = nowt
                try:
                    from mawitek.infra.heartbeat import stale_heartbeats
