...
def _fetch_for_ticker(ticker: str, max_articles: int = MAX_HEADLINES_PER_TICKER) -> list[dict]:
    ...
    """
    try:
        from mawitek.data.news_sources import fetch_all_news
...
def _focus_tickers(max_n: int = MAX_FOCUS_TICKERS) -> list[str]:
    ...
    try:
        from mawitek.core.risk_manager import _open_underlyings
    ...
    while True:
        try:
            ...
            if sweeps % SOCIAL_POLL_EVERY == 0:
                try:
                    from mawitek.data.social_sentiment import social_sweep_once
