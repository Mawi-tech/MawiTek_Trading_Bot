...
import json
import os
from mawitek.data.tradier_client import get_account_balance, get_open_positions
from mawitek.infra.utils import now_est, today_est
from mawitek.infra.state_io import atomic_write_json, file_lock
...
def active_config(equity: float) -> dict | None:
    ...
    """
    try:
        from mawitek.core.user_config import effective_config
...
def _vega_reject(strategy: str | None, equity: float) -> str | None:
    ...
    if not PORTFOLIO_VEGA_LIMIT or equity <= 0:
        return None
    try:
        from mawitek.core.portfolio_greeks import cached_net_vega
...
def calculate_daily_pnl() -> float:
    ...
    from mawitek.core.equity_tracker import get_live_equity, get_baseline_equity_for_today
...
def check_daily_loss_limit(equity: float) -> tuple[bool, float]:
    ...
    if halt_now:
        ...
        # must still register.
        try:
            from mawitek.infra.event_notifier import notify_halt_triggered
...
def _bear_throttle(strategy: str | None, equity: float = 0.0) -> tuple[float, float, str | None]:
    ...
    if not BEAR_REGIME_THROTTLE or not tier_throttle or strategy in BEAR_THROTTLE_EXEMPT:
        return 1.0, 1.0, None
    try:
        from mawitek.data.market_regime import is_bear_market
...
def pre_trade_check(ticker: str, strategy: str | None = None) -> dict:
    ...
    try:
        from mawitek.core.bot_control import control_block_reason
...
def reconcile_positions_from_broker() -> int:
    ...
    from mawitek.core.position_manager import load_positions, remove_position
