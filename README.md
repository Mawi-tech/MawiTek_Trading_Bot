...
> **New to the terminology?** [docs/GLOSSARY.md](docs/GLOSSARY.md) explains every scanner signal, score, and data source in plain English.
...
| **Fail-closed state reads** | A `risk_state.json` or `pending_orders.json` that exists but won't parse is quarantined to `<path>.corrupt.<ts>` and raises `StateCorruption` instead of quietly reading as `{}` — which would mean *halt flag gone, daily P&L zeroed, nothing in flight*. The strategy logs at ERROR, alerts, and refuses to start. A stopped bot is recoverable; one trading on a blank risk state is not. |
| **Lock ownership tokens** | Each `file_lock` acquisition stamps `<pid>:<uuid4>` into the lock file. Release removes it only if the token still matches, and a stale lock is broken only if its contents didn't change between the check and the removal — so two processes can never both believe they hold it. Long holds warn. |
...
| **Dashboard server** | `dashboard_server.py` enforces an **exact-filename** allowlist (`dashboard.html / backtest_dashboard.html / dashboard.css / dashboard.js / chart.umd.js` + `dashboard_state.json / backtest_equity.json / news_feed.json / social_sentiment.json`) so the static server can't serve `.env`, a source map, an editor backup, or any other state file. Optional HTTP Basic Auth (`DASH_AUTH_USER`/`DASH_AUTH_PASS`). Loopback-only by default, and it **refuses to start** on a non-loopback `--bind` unless both auth variables are set — an exposed `/api/control` is a kill switch. |
