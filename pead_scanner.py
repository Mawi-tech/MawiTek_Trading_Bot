...
from market_regime import current_regime, is_bear_regime, REGIME_SMA_DAYS, REGIME_TICKER
...
def _fetch_daily_cached(ticker: str, days: int) -> pd.DataFrame:
    ...
    if cached is not None and (time.time() - cached[0]) < DAILY_CACHE_TTL_SEC:
        return cached[1]
    from market_data import get_daily_bars   # local import keeps detectors import-light
